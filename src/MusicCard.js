...
function MusicCard({data}){
    ...
    const {setSongData} = useContext(JiosaavanContext);
    ...
    return (
        <div className='music-card-container' onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
            <div className='music-card-img' style={data.type == "artist" ? {backgroundImage:`url(${data.image || data.thumbnail})`, borderRadius: "50%"} : {backgroundImage:`url(${data.image || data.thumbnail})`}}>
                {
                    isHover &&
                    <div className='music-card-controls' onClick={(e) => handleMusicCardControls(e)}>
                        ...
                        <div className='music-card-heart-ellipsis'>
                            {isLikeRed ? <AiFillHeart className='music-card-like-btn' style={{color: 'red'}} /> : <FontAwesomeIcon className='music-card-like-btn' icon={faHeart} style={{color: '#fff'}} />}
